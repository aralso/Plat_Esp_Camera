...
#define DCT1D(s0, s1, s2, s3, s4, s5, s6, s7) \
    ...
    {
        DCT2D(m_sample_array); // output is in: m_sample_array

	//static int COUNT2 = 0;
	//mcu_coefs_t coefs2(m_sample_array);
	//if (COUNT2++ > 150 && COUNT2 < 160)
	//	coefs2.dump("B");

        load_quantized_coefficients(component_num); // output is in: m_coefficient_array
