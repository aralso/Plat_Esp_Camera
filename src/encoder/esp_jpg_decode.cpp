...
//static uint32_t _jpg_read(JDEC *decoder, uint8_t *buf, uint32_t len)
{
    ...
    if (len) {
        ...
        if (!len) {
            if (log_detail >= 1) ESP_LOGE(TAG, "Read Fail at %u/%u", jpeg->index, jpeg->len);
        }
        jpeg->index += len;
    }
    return len;
...
esp_err_t esp_jpg_decode(size_t len, jpg_scale_t scale, jpg_reader_cb reader, jpg_writer_cb writer, void * arg)
{
    ...
    if(jres != JDR_OK){
        if (log_detail >= 1) ESP_LOGE(TAG, "JPG Header Parse Failed! %s", jd_errors[jres]);
        return ESP_FAIL;
    }
...
#if JPGE_VERBOSE
    printf("Header has %ld bytes\nJPEG is %d by %d\n", jpeg.index, decoder.width, decoder.height);
#endif
    ...
    if (jres != JDR_OK) {
        if (log_detail >= 1) ESP_LOGE(TAG, "JPG Decompression Failed! %s", jd_errors[jres]);
        return ESP_FAIL;
    }
