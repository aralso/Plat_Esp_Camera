...
)
{
  ...
  signed short m_coefficient_array[64];
  ...
  for (blk = 0; blk < nby + nbc; blk++) {
	...
	m_coefficient_array[i] = d;
        tmp[z] = d * dqf[z] >> 8;   /* De-quantize, apply scale factor of Arai algorithm and descale 8 bits */
      }
    } while (++i < 64);   /* Next AC element */

	if (dump_blk)
      printf("> DUMPING MCU FOR BLK [%d/%d]\n", 1+blk, nby + nbc);

    if (dump_blk)
    {
        printf("\t>>DUMP AFTER HUFF EXTRACT\n");
        for (int j = 0; j < 8; j++)
        {
...
static JRESULT mcu_output (
  JDEC* jd,   /* Pointer to the decompressor object */
  unsigned (*outfunc)(JDEC*, void*, JRECT*),  /* RGB output function */
...
#if JPGE_VERBOSE
	  bool dump = x/8 == 0 && y/8 == 0;
#else
	  bool dump = false;
#endif
      rc = mcu_load(jd, dump);          /* Load an MCU (decompress huffman coded stream and apply IDCT) */
      if (rc != JDR_OK) return rc;
